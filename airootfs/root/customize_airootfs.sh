...
mkdir -p /home/arch/.config/spectre/scripts
[ -f /home/arch/.config/spectre/scripts/wallpaper-hypr.sh ] && chmod +x /home/arch/.config/spectre/scripts/wallpaper-hypr.sh
...
# Enable SDDM and boot intro
systemctl enable sddm.service
systemctl enable spectre-boot-intro.service
systemctl set-default graphical.target 2>/dev/null || true

# Boot intro runs before SDDM - add override so SDDM waits
mkdir -p /etc/systemd/system/sddm.service.d
printf '%s\n' '[Unit]' 'After=spectre-boot-intro.service' > /etc/systemd/system/sddm.service.d/after-intro.conf
